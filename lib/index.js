...
let mailTransporter = nodemailer.createTransport({
    service: "gmail",
    auth: {
        user: "ryanangelo.delacruz@gsfe.tupcavite.edu.ph",
        pass: "bbnbnhhg"
    }
})
    ...
    db.query(query, [email], (err, results) => {
        if(results.length === 0){
            ...
            try {
                ...
                //generate mail
                let details = {
                    from: "ryanangelo.delacruz@gsfe.tupcavite.edu.ph",
                    to: email,
                ...
                mailTransporter.sendMail(details, (err) => {
                    try {
                        res.status(200).json({message: 'We have just sent you the link to reset your password. Please check your email account.'})
                    } catch (error){
